...
```bash
# 创建证书目录
mkdir -p /home/lanser/aiprj/zMessage/data/certs

# 生成自签名证书
openssl req -x509 -newkey rsa:4096 -keyout /home/lanser/aiprj/zMessage/data/certs/key.pem \
  -out /home/lanser/aiprj/zMessage/data/certs/cert.pem -days 365 -nodes \
  -subj "/C=CN/ST=Beijing/L=Beijing/O=zMessage/CN=localhost"

# 验证证书
openssl x509 -in /home/lanser/aiprj/zMessage/data/certs/cert.pem -text -noout
```
...
# 验证目录结构
tree /home/lanser/aiprj/zMessage -L 3 -I 'node_modules'
```
...
if [ -d "/home/lanser/aiprj/zMessage" ]; then
    echo "项目目录存在"
    ls -la /home/lanser/aiprj/zMessage
else
    echo "项目目录不存在"
fi
...
if [ -f "/home/lanser/aiprj/zMessage/go.mod" ]; then
    echo "go.mod 已存在"
    cat /home/lanser/aiprj/zMessage/go.mod
else
...
```bash
# 确保 data 目录有写权限
chmod -R 755 /home/lanser/aiprj/zMessage/data
```
